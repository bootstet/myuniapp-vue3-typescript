<template>
	<mlUniPopup type="bottom" percent="63%" ref="popup">
		<view class="region-container">
			<view class="title">
				请选择
				<image @click="close" c-click class="close" src="/static/image/icon_close.png"></image>
			</view>
			<view class="nav-list" >
				<block v-for="addressKey,i in addressKeys" :key="i">
					<view class="item line1" :class="{'selected':navKey==addressKey}" @click="changeNav(addressKey)" c-click v-if="selectedAddress[addressKey]!=null">{{selectedAddress[addressKey].name}}</view>
				</block>
				
				<view class="item line1 selected" v-if="!finish&&selectedAddress.street==null" @click="asd" c-click>请选择</view>
			</view>
	...
	methods:{
		...
		getCitys(city_id){
			return new Promise((reslove, reject) => {
				uni.request({
					url: 'http://localhost:1080/get_json',
					method:  'GET',
